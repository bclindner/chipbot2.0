...
class KarmaVoting extends RegexCommand {
  ...
  run (msg) {
    ...
    let subjectName = msg.content.slice(0, -2).trim() // the subject
    // get our subject object
    let subject = this.karma.get(subjectName.toLowerCase())
    // determine what to do given the symbol
    switch (symbol) {
      case '++':
        if (subject.canUpvote) {
          subject.upvote()
          msg.reply('upvote successful: ' + subjectName + ' now has ' + subject.points + ' karma.')
        } else {
      ...
      case '--':
        if (subject.canDownvote) {
          subject.downvote()
          msg.reply('downvote successful: ' + subjectName + ' now has ' + subject.points + ' karma.')
        } else {
          // if there is ridicule, then ridicule them
          if (subject.canRidicule) {
            msg.reply(subject.getRandomRidicule())
